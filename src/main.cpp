...
static std::vector<Preset> build_presets() { //not all of these are common
    std::vector<Preset> p;
...
static void render_viewport(const ViewerState& s, const Preset& preset, int rows,
                            std::vector<uint8_t>& out_pixels, uint32_t& out_rows_rendered) {
    const auto total_bits = s.data.size() * 8;
    const auto start_bit = s.start_offset_bytes * 8 + s.bit_align;
    if (start_bit >= total_bits) {
        out_rows_rendered = 0;
        out_pixels.clear();
        return;
    }
    const auto width = std::max<uint32_t>(1, s.width_px);
    const auto pixels_to_render = rows * width;
    size_t pixels_available = (total_bits - start_bit) / s.bpp;
    if (pixels_available == 0) {
        out_rows_rendered = 0;
        out_pixels.clear();
        return;
    }
    const auto actual_pixels = std::min<uint32_t>(pixels_to_render, pixels_available);
    ...
    for (uint32_t p = 0; p < rows_needed * width; ++p) {
        ...
        for (auto &f : preset.fields) {
            const int use = std::min(f.bits, cur_shift);
            uint64_t rawcomp = 0;
...
static bool save_png(const std::string &filename, int w, int h, const std::vector<uint8_t>& buf) {
...
static bool load_file_into(ViewerState &S, const char *path) {
    ...
    std::vector<uint8_t> tmp((size_t)sz);
    in.read(reinterpret_cast<char *>(tmp.data()), sz);
    S.data.swap(tmp);
    S.filename = path;
    S.start_offset_bytes = 0;
    S.bit_align = 0;
    return true;
}
...
int main(int argc, char** argv) {
    ...
    if (!gl_context) {
        std::fprintf(stderr, "Error: SDL_GL_CreateContext failed: %s\n", SDL_GetError());
        return 1;
    }
    ...
    while (!want_quit) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_DROPFILE) {
                char* dropped_filedir = event.drop.file; // must free with SDL_free()
                if (dropped_filedir) {
                    std::strncpy(pathbuf, dropped_filedir, sizeof(pathbuf)-1);
            ...
            if (event.type == SDL_KEYDOWN && !io.WantCaptureKeyboard) {
                ...
                if (k == SDLK_LEFT) { S.width_px = std::max<size_t>(1, S.width_px - 1); }
                else if (k == SDLK_RIGHT) { S.width_px = S.width_px + 1; }
                else if (k == SDLK_UP) { S.start_offset_bytes = (S.start_offset_bytes>0) ? S.start_offset_bytes - 1 : 0; }
                else if (k == SDLK_DOWN) { S.start_offset_bytes = S.start_offset_bytes + 1; }
                else if (k == SDLK_PAGEUP) {
                    // compute visible rows
                    int win_w, win_h;
                    SDL_GetWindowSize(window, &win_w, &win_h);
                    int image_h = std::max(1, win_h);
                    ...
                    auto start_bit = static_cast<size_t>(S.start_offset_bytes) * 8 + S.bit_align;
                    size_t nstart = start_bit - page_bits;
                    if (nstart < 0) nstart = 0;
                    S.start_offset_bytes = nstart / 8;
                    S.bit_align = (int)(nstart % 8);
                }
                else if (k == SDLK_PAGEDOWN) {
                    int win_w, win_h;
                    SDL_GetWindowSize(window, &win_w, &win_h);
                    int visible_rows = std::max(1, win_h);
                    int visible_pixels = S.width_px * visible_rows;
                    int visible_bits = visible_pixels * S.bpp;
                    int page_bits = (visible_bits * 2) / 3;
                    auto start_bit = static_cast<size_t>(S.start_offset_bytes) * 8 + S.bit_align;
                    size_t nstart = start_bit + page_bits;
                    if (auto total_bits = S.data.size() * 8; nstart > total_bits - S.bpp)
                        nstart = std::max<size_t>(0LL, total_bits - S.bpp);
                    S.start_offset_bytes = nstart / 8;
                ...
                if (event.key.keysym.mod & KMOD_SHIFT) {
                    if (k == SDLK_UP) {
                        ...
                        S.bit_align = std::max<uint8_t>(0, S.bit_align - 1);
                    } else if (k == SDLK_RIGHT) {
                        S.bit_align = std::min<uint8_t>(7, S.bit_align + 1);
                    }
                }
            }
        }

        // Start the Dear ImGui frame (note: current SDL backend expects no window argument)
        ...
        if (ImGui::Button("Browse...")) {
            ...
            if (nfdresult_t result = NFD_OpenDialog(&outPath, nullptr, 0, nullptr); result == NFD_OKAY) {
                std::strncpy(pathbuf, outPath, sizeof(pathbuf)-1);
                pathbuf[sizeof(pathbuf)-1] = '\0';
        ...
        ImGui::InputInt("Start ofs (bytes)", &S.start_offset_bytes);
        ...
        int display_w = (int)avail.x;
        int display_h = (int)avail.y;
        if (display_w < 1) display_w = 64;
        if (display_h < 1) display_h = 64;

        // perform deferred load if requested
        if (load_requested) {
            if (!load_file_into(S, pathbuf)) {
                std::cerr << "Failed to open file: " << pathbuf << std::endl;
            }
        ...
        if (save_requested && rows_rendered > 0) {
            std::string outname = "rawviewer_out.png";
            if (save_png(outname, tex_w, tex_h, pixels)) {
                std::cerr << "Saved " << outname << std::endl;
            } else {
        ...
        // Render ImGui
        ImGui::Render();
        int fb_w = (int)io.DisplaySize.x;
        int fb_h = (int)io.DisplaySize.y;
